class Mobile::V1::RidesController < ApplicationController
	...
	def user_rides_information
		...
		@user_with_may_be = @user.user_rides.where(perticipate_ride:"May Be").collect { |user_ride| Ride.find(user_ride.ride_id) }
	...
	def ride_with_user_information
		...
		@ride_may_be_users = @ride.user_rides.where(perticipate_ride:"May Be").collect { |ride_user| User.find(ride_user.user_id) }
